...
def test_cmyk_pdf_transformation():
    if not os.environ.get("TEST_CMYK_PDF_ENABLED", False):
        return
    ...
    for filename in os.listdir(input_files):
        if filename.endswith(".pdf"):
            ...
            assert (abs(output_pdf_size - expected_pdf_size) / expected_pdf_size) < 0.01

            # Compare length
            assert len(test_pdf_value) == len(expected_pdf_value)
